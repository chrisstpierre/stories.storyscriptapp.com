when github events webhooks as webhook
    if webhook.event == 'push'
        ...
        service = psql exec query: 'SELECT services.uuid FROM app_public.services
                                    INNER JOIN app_hidden.repos ON repos.uuid = services.repo_uuid
                                    WHERE repos.service_id = cast({webhook.payload['repository']['id']} as text)
                                    LIMIT 1;'
        if service
